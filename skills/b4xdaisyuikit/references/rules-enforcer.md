...
* **`RULE-SETUP-005` (Asset Bundling via .b4xlib)**: `B4XDaisyUIKit.b4xlib` delivers all default assets (SVG icons, fonts, logo) automatically into `File.DirAssets` at compile time. Do NOT declare default `.b4xlib` assets as `FileN=` in `<AppName>.b4a`.
