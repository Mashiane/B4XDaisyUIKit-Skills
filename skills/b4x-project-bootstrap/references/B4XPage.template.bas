...
Private Sub B4XPage_Resize(iWidth As Int, iHeight As Int)
	If pageScroll.IsInitialized Then
		pageScroll.Base_Resize(iWidth, iHeight)
...
Public Sub IME_HeightChanged(iNewHeight As Int, iOldHeight As Int)
	Try
		...
		If iNewHeight < iOldHeight Then
			...
			pageScroll.SetLayoutAnimated(0, 0, 0, Root.Width, iNewHeight)
			Sleep(50)
		...
		End If
	Catch
		If B4XDaisyApp.DebugLogs Then Log("B4XPage.IME_HeightChanged: " & LastException.Message)
	End Try
End Sub
...
Public Sub ScrollFocusedInputIntoView
	...
	Catch
		If B4XDaisyApp.DebugLogs Then Log("B4XPage.ScrollFocusedInputIntoView: " & LastException.Message)
	End Try
End Sub
...
Private Sub HandleInputFocus(bHasFocus As Boolean)
	Try
		...
		End If
	Catch
		If B4XDaisyApp.DebugLogs Then Log("B4XPage.HandleInputFocus: " & LastException.Message)
	End Try
End Sub
