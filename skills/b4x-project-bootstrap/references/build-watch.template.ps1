...
if ($disp.Success) {
    $dispMs = [int]$disp.Groups[1].Value
    if ($dispMs -ge 3000) {
        $errors += "Startup: Displayed = ${dispMs} ms (budget 2000 ms, hard cap 3000 ms)"
    } elseif ($dispMs -ge 2000) {
        $warnings += "Startup: Displayed = ${dispMs} ms (exceeds 2000 ms budget)"
    } else {
        $verified += "Startup: ActivityManager Displayed = ${dispMs} ms (budget 2000 ms)"
    }
} else {
...
$report += "Budgets: startup 2000ms (cap 3000ms) | heavy jank >= 20 skipped frames | touch >= 48dp | crash/ANR = error."
